...
class Erc20Service:
    ...
    def transfer(self, wallet_client: EVMWalletClient, parameters: dict):
        try:
            ...
            tokenAddress = wallet_client.resolve_address(parameters["tokenAddress"])
            print(tokenAddress,parameters["tokenAddress"])
            print(f"Wallet Address: {wallet_client.get_address()}")
            print(f"Wallet Chain: {wallet_client.get_chain()}")
            print(f"Wallet Balance: {wallet_client.balance_of(wallet_client.get_address())}")
            ...
            hash_result = wallet_client.send_transaction(
                {
                    ...
                    "args": [to_address, int(parameters["amount"])],
                }
            )
            ...
            hash_result = wallet_client.send_transaction(
                {
                    ...
                    "args": [spender, int(parameters["amount"])],
                }
            )
            return hash_result["hash"]
        except Exception as error:
            ...
            hash_result = wallet_client.send_transaction(
                {
                    ...
                    "args": [wallet_client.get_address(), to_address, int(parameters["amount"])],
                }
            )
            return hash_result["hash"]
        except Exception as error:
